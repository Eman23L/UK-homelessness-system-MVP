...
from parsers.medical_parser import fetch_medical_services
...
def clear_existing_medical_rows(conn: sqlite3.Connection) -> None:
    cursor = conn.cursor()
    cursor.execute("DELETE FROM services WHERE provider_name = ?", ("nhs_ods",))
    conn.commit()
...
def main() -> None:
    print("Fetching medical services...")
    services = fetch_medical_services()
    print(f"Fetched {len(services)} medical services.")

    conn = get_connection()
    try:
        ensure_columns(conn)
        clear_existing_medical_rows(conn)
        insert_services(conn, services)
    finally:
        conn.close()

    print("Medical import complete.")
